...
Define the structure of entities representing pattern examples, categories, and navigation—all hardcoded in source code (no database persistence).
...
| `category`                | ExampleCategory | Yes      | Category reference                        | Must match valid ExampleCategory.id               |
| `description`             | string          | Yes      | Brief explanation of change               | Max 200 chars; plain text                         |
| `whyItMatters`            | string          | Yes      | Rationale for pattern change              | Max 300 chars; plain text                         |
...
None—examples are static data; no lifecycle states.
...
Not applicable—examples are hardcoded. Future expansion (e.g., CMS integration) would require:
